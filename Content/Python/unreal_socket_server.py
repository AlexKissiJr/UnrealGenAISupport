...
def stop_server():
    ...
    try:
        ...
        log.log_info("Command processor unregistered")
    except Exception as e:
        log.log_error(f"Error unregistering command processor: {str(e)}")

    # Note: We can't directly stop the thread, but it will be cleaned up when the Python module is reloaded
    log.log_info("Socket server stopping")
    return True
