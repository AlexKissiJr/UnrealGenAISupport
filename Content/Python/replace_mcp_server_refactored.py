...
def replace_mcp_server():
    """Replace the standard mcp_server.py with our WebSocket refactored version"""
    try:
        ...
        mcp_server_refactored_path = os.path.join(python_path, "mcp_server_refactored.py")
        mcp_server_backup_path = os.path.join(python_path, "mcp_server_original.py")

        if not os.path.exists(mcp_server_refactored_path):
            unreal.log_error(f"Refactored WebSocket version of mcp_server.py not found at: {mcp_server_refactored_path}")
            return False
        ...
        # Replace mcp_server.py with our refactored WebSocket version
        unreal.log(f"Replacing mcp_server.py with refactored WebSocket version")
        shutil.copy2(mcp_server_refactored_path, mcp_server_path)

        unreal.log("✅ Successfully replaced mcp_server.py with refactored WebSocket version")
        return True
...
if __name__ == "__main__":
    unreal.log("Replacing mcp_server.py with refactored WebSocket version...")
    success = replace_mcp_server()

    if success:
        unreal.log("✅ Successfully replaced mcp_server.py with refactored WebSocket version")
    else:
        unreal.log_error("❌ Failed to replace mcp_server.py with refactored WebSocket version")

# Functions to run from Unreal Engine
def replace():
    """Replace the standard mcp_server.py with our refactored WebSocket version"""
